...
7. **Arbitrary Pixel Typography (`text-[px]`), Overuse of `text-2xs`, & Unsanctioned `font-mono`:** Never use arbitrary pixel text utilities like `text-[10px]`, `text-[12px]`, or `text-[14px]`. Do not overuse micro typography like `text-2xs` (0.65rem/10px) — default to standard `text-xs` or `text-sm` for UI controls, body text, and dialog descriptions to ensure high legibility. Never use `font-mono` for general UI text, file sizes, timestamps, or metadata; use clean sans typography with `tabular-nums` instead, reserving monospace strictly for code blocks, keyboard shortcuts, or technical hashes.
