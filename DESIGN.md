...
* **No Monospace Typography (`font-mono`) in General UI:** Do not use `font-mono` for general UI text, file sizes, dimensions, timestamps, or card metadata. Use standard clean sans-serif typography (`font-sans` / default) with tabular numerals (`tabular-nums`) where numeric alignment is needed. Reserve monospace strictly for code blocks (`<code>`), keyboard shortcuts (`<kbd>`), and technical hashes/checksums.
