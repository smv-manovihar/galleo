...
import { buildFolderPathFromPattern, resolveFilenameConflict, planOrganization } from '../organization';
...
describe('buildFolderPathFromPattern', () => {
  ...
  it('handles month names (MMMM)', () => {
    expect(buildFolderPathFromPattern('YYYY/MM - MMMM/DD', date)).toBe('2024/03 - March/15/');
  });
...
describe('planOrganization', () => {
  const createMockItem = (id: string, dateStr: string, name: string): MediaItem => ({
    id,
    path: `D:\\Source\\${name}`,
    name,
    size: 2000,
  ...
  it('plans organization paths and flags conflicts', () => {
    const item1 = createMockItem('item1', '2024-03-15T12:00:00.000Z', 'photo.jpg');
    const item2 = createMockItem('item2', '2024-03-15T12:00:00.000Z', 'other.jpg');
