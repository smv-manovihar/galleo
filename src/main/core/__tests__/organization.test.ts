...
describe("planOrganization", () => {
  ...
  it("plans organization without destinationDir for dry-run preview", () => {
    ...
    expect(plan[0].dateTarget).toBe("2024-03-15T12:00:00.000Z")
    expect(plan[0].dateTargetSource).toBe("filesystem")
    expect(plan[0].size).toBe(2000)
    expect(plan[0].mediaType).toBe("photo")
  })
})
