...
interface ScoreParams {
  ...
  peakBrightness?: number // 0 - 255 (95th percentile peak brightness)
...
export function evaluateQuality(params: ScoreParams): QualityMetrics {
  ...
  // A photo is considered dark only if:
  // 1. Its average brightness is below the darknessThreshold.
  // 2. Its peak brightness (95th percentile) is low (below 120) to ensure there are no bright subjects/highlights.
  // 3. Its contrast is low (below 100) to ensure it's not a high-contrast graphic or dark mode scene.
  const isDark =
    brightness < thresholds.darknessThreshold &&
    (peakBrightness === undefined || peakBrightness < 120) &&
    (contrast === undefined || contrast < 100)

  const isBlurry = blurScore < thresholds.blurThreshold

  const isScreenshot =
    thresholds.screenshotDetection && isScreenshotByName(filename)
