...
export interface DecisionHistoryDialogItem {
...
const HistoryRow = memo(
  ({
    ...
    index: number
    item: DecisionHistoryDialogItem
    isChecked: boolean
    isKeep: boolean
    isDelete: boolean
    onSingleAction: (
    ...
    onPreview: (item: DecisionHistoryDialogItem) => void
    translateY: number
...
// Isolated component that owns the virtualizer so scroll-driven re-renders
// don't propagate up to the dialog shell (header, bulk bar, etc.).
const VirtualizedHistoryList = memo(
  ({
    ...
    items: DecisionHistoryDialogItem[]
    ...
    return (
      <>
        ...
        {/* Scroll container — only this subtree re-renders on scroll */}
        <div
...
export interface DecisionHistoryDialogProps {
  isOpen: boolean
...
export const DecisionHistoryDialog: React.FC<
