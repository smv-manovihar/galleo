  ...
  const renderActionButtons = () => {
    ...
    const moreButton = (
      <DropdownMenu>
        ...
        <DropdownMenuContent
          align="end"
          className="w-44 border-border bg-card font-sans text-sm text-foreground shadow-xl"
        >
          <DropdownMenuItem
    ...
    return (
      ...
      >
        {/* Keep */}
        <Tooltip>
          <TooltipTrigger asChild>{keepButton}</TooltipTrigger>
          <TooltipContent>
      ...
      <CardContent className="relative flex aspect-square flex-col justify-end bg-muted/20 p-0">
        ...
        {hasQuality && (
          <div className="absolute top-2 left-2 z-20">
            <QualityScoreBadge item={item} />
          </div>
        )}

        {/* Matching Video Timestamp Badge */}
        {isVideo && matchingFrame && (
          <div className="absolute top-2 left-2 z-20 flex items-center gap-1 rounded-md bg-purple-700 px-2 py-0.5 text-xs font-semibold text-white shadow-md">
            <span>
              ⏱{" "}
        ...
        {isVideo && (
          <div className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center">
            <div
              className="pointer-events-auto flex h-10 w-10 cursor-pointer items-center justify-center rounded-full border border-white/25 bg-black/75 text-white shadow-md transition-all duration-200 group-hover:scale-110 group-hover:border-primary/50 group-hover:bg-primary group-hover:text-primary-foreground"
              onClick={(e) => {
                e.stopPropagation()
                if (onPlayOpen) {
                  onPlayOpen(item)
