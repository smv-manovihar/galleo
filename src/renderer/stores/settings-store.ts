...
export const useSettingsStore = create<SettingsState>((set, get) => ({
  ...
  addRootFolder: async (folderPath: string) => {
    ...
    const isAlreadyCovered = settings.folders.roots.some(r => {
      ...
      return existingNormalized === newNormalized || newNormalized.startsWith(existingNormalized + '/');
    });
    ...
    const subPathsToRemoveSet = new Set(subfoldersToRemove.map(s => s.path.toLowerCase()));
  ...
  removeRootFolder: async (folderPath: string) => {
    ...
    if (updatedRoots.length === 0) {
      nextActive = null;
    } else if (!currentActive || currentActive.toLowerCase() === folderPath.toLowerCase()) {
      nextActive = "all";
    }
