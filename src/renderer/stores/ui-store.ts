...
interface UIState {
  ...
  activeSettingsTab: 'folders' | 'scan' | 'quality' | 'appearance' | 'reset' | 'about';
  ...
  setActiveSettingsTab: (tab: 'folders' | 'scan' | 'quality' | 'appearance' | 'reset' | 'about') => void;
...
export const useUIStore = create<UIState>((set, get) => ({
  ...
  dismissedVersion: typeof window !== 'undefined' ? localStorage.getItem('galleo_dismissed_update') : null,
  ...
  checkForUpdates: async () => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        const errorMsg = result.error.code === 'UNKNOWN' ? result.error.message : `Check for updates failed (${result.error.code})`;
        set({
